...
**Two ways metrics reach Prometheus — push vs pull** (don't assume every module uses OTLP):
- **Push (default, most modules):** OTLP starter → collector → Prometheus scrapes the collector's `:8889`. The single `otel-collector` job in `monitoring/prometheus.yml` covers them all; a new push-mode module needs **no** Prometheus change.
- **Pull (`creed-simple-metrics`, port `8096`):** depends on `micrometer-registry-prometheus` (no OTLP starter) and exposes `/actuator/prometheus` itself. Prometheus must scrape it **directly** with its own `scrape_configs` job. Three gotchas in that job: `scheme: https` (the app is HTTPS), `tls_config.insecure_skip_verify: true` (Creed-CA self-signed), and `targets: ['host.docker.internal:8096']` (Prometheus is containerized — `localhost` won't reach the host app). The actuator path is `/actuator/prometheus`, **not** under any servlet context-path (e.g. simple-metrics' Camel `/camel/*` does not apply). After editing `monitoring/prometheus.yml`, **restart** Prometheus (`docker compose -f monitoring/docker-compose.yml restart prometheus`) — the container has no `--web.enable-lifecycle`, so `/-/reload` is unavailable. Verify with `up{job="..."}` == 1.
...
**`tomcat_connections_keepalive_current` does NOT mean "idle connections"** — Tomcat's NIO `keepAliveCount` counts every poller-registered socket, *including* ones with a request in flight. Measured under k6 load on simple-metrics: `current=140, keepalive=139, busy=50` (i.e. `keepalive == current - 1` at saturation, the `-1` being the acceptor's pre-acquired `LimitLatch` slot). So `current - keepalive` is ~1 always, and any "active connections" formula built on it goes **negative exactly when the connector saturates** — the opposite of the intended signal. Tomcat exposes no accept-queue gauge at all; to estimate request backlog use `(connections.current - threads.busy)` **gated on `threads.busy >= threads.config.max`** (`* on (name) group_left () (busy >= bool config_max)`), since Tomcat can only queue once every worker is taken and the gate is what suppresses idle keep-alive sockets reading as phantom backlog. That's panel 705 of `otel-jvm-statistics.json`.

**Don't guess names — read them live**: `curl -s localhost:8889/metrics | grep <prefix>` (collector scrape), and validate PromQL via `curl -s localhost:9090/api/v1/query --data-urlencode 'query=...'`. Dashboards are provisioned, so editing `monitoring/dashboards/*.json` + `docker compose -f monitoring/docker-compose.yml restart grafana` reloads them (verify uid via `/api/dashboards/uid/<uid>`). Main dashboard: `otel-jvm-statistics.json` (uid `otel-jvm-statistics`). Grafana's admin password is no longer the compose default `admin`, so the API needs real creds (repeated 401s temporarily lock the admin login) — validate panel queries against Prometheus directly instead.

**Local (non-header) baggage fields don't survive every span-creation path**: a Brave `SingleBaggageField.local()` (deliberately not injected onto outbound headers — see [[creed-simple-metrics]]'s `MyMDCScopeDecorator`/`correlationTraceId`) only survives where a library actually threads Brave's ambient `CurrentTraceContext`. Libraries with their own parent-span lookup (camel-tracing/camel-observation's `ActiveSpanManager`/`ObservationRegistry.getCurrentObservation()`, built for cross-process header-based propagation) don't reliably carry it into a new child span/Observation — confirmed root cause of a real bug (fixed by removing `camel-observation-starter` entirely, costing its route-level Prometheus timers as a documented tradeoff). Full investigation + a reusable methodology (bracket the suspected boundary with diagnostic processors reading the *raw* field value, A/B toggle suspected components instead of trusting static analysis) in `creed-simple-metrics/docs/camel-observation-baggage-loss.md`. Relevant to any future module that adopts local baggage fields alongside a non-Brave-native tracing/instrumentation layer.
