...
- A bundle is only materialized when something reads it (`SslBundles.getBundle`) — but **three things read every declared bundle eagerly**: `SslMeterBinder` (autoconfiguration `org.springframework.boot.micrometer.metrics.autoconfigure.ssl.SslMetricsAutoConfiguration`, renamed in Boot 4 — the old `actuate.autoconfigure.ssl.SslObservabilityAutoConfiguration` exclusion silently stopped matching), the SSL health indicator (`management.health.ssl.enabled`), and — new in Boot 4 — the actuator's `SslInfo` (`management.info.ssl.enabled`). A declared-but-missing keystore therefore still fails startup even with HTTPS off; test/dev profiles must disable all three.
- **The PKI is not in git.** `.support/scripts/pki/` is generated by `.support/scripts/CA-Generation.sh`, and the script writes to `./pki` **relative to its own cwd** — run it from `.support/scripts/`, not the repo root. `creed-gateway/src/main/resources/certs/` (the `cloud` profile's config-client truststore) is not in git either, so the gateway's `cloud` profile cannot complete its config-server bootstrap on a fresh clone.
...
## Spring Boot 4 module split

Boot 4 broke the monolithic starters apart; what used to arrive transitively must now be declared.
- `spring-boot-starter-actuator` no longer brings tracing → add **`spring-boot-micrometer-tracing`**, else no `io.micrometer.tracing.Tracer` bean and the context fails.
- `spring-boot-starter-test` no longer bundles MockMvc/TestRestTemplate → **`spring-boot-starter-webmvc-test`**, **`spring-boot-resttestclient`** (or `spring-boot-starter-test-classic`).
- `spring-boot-starter-web` no longer brings RestTemplate/RestClient → **`spring-boot-starter-restclient`**.
- Moved classes worth remembering: `boot.DefaultBootstrapContext`→`boot.bootstrap.*`, `boot.web.embedded.tomcat.TomcatServletWebServerFactory`→`boot.tomcat.servlet.*`, `boot.web.client.RestTemplateBuilder`→`boot.restclient.*`, `boot.actuate.metrics.MetricsEndpoint`→`boot.micrometer.metrics.actuate.endpoint.*`, `boot.actuate.health.HealthEndpoint`→`boot.health.actuate.endpoint.*`, `ClientHttpRequestFactorySettings`→`HttpClientSettings`.
- **Don't guess a new package** — index the jars: `for j in $(find ~/…/repos/org/springframework -name '*-4.0.8.jar'); do unzip -l $j | grep '/<SimpleName>.class'; done`.
- **Jackson 3** (`tools.jackson.*`): exceptions are unchecked (`JacksonException`), so `catch (IOException)`/`throws JsonProcessingException` around mapper calls becomes dead code; mappers are immutable (`JsonMapper.builder()…build()`); `JavaTimeModule` is gone (java.time is built in). Jackson 2 stays BOM-managed for third-party libraries that need it.
- **A reactive module cannot use the servlet HTTPS customizer.** `creed-gateway` is WebFlux/Netty and had a copy of the servlet modules' `TomcatHttpsConfiguration`; typed on `TomcatServletWebServerFactory` it never matched, so its SSL bundle was silently never applied. It is now `ReactiveHttpsConfiguration`, typed on `ConfigurableReactiveWebServerFactory`.
...
**Pool metrics**: use **`com.creed.metrics.ConnectionPoolMetrics(cm, "<name>")`** from `creed-common-metrics` (a `MeterBinder`; the name becomes the `httpclient` tag). Metrics: `httpcomponents.httpclient.pool.total.{max,connections[state],pending}` + `route.max.default`. Do **not** hand-roll gauges anywhere else — this class is the one sanctioned copy.

It replaces Micrometer's `PoolingHttpClientConnectionManagerMetricsBinder`, deprecated together with the whole `io.micrometer.core.instrument.binder.httpcomponents.hc5` package from **Micrometer 1.17 / Boot 4.1** in favour of Apache's `httpclient5-observation`. That artifact is **not** a drop-in for the pool half, for two reasons worth remembering before anyone "finishes" the migration:
- its `ConnPoolMeters` emits `<prefix>.pool.{leased,available,pending}` — three separate gauges, no `state` tag — and has **no** `pool.total.max` / `pool.route.max.default` at all, so the capacity series panel 705 needs would vanish even with `MetricConfig.prefix` overridden;
- its `bindTo` takes an `HttpClientBuilder`, whereas every pool here is bound as a standalone `PoolingHttpClientConnectionManager` (Camel's `HttpComponent`, the partner gateway's dynamic clusters).

`ConnectionPoolMetrics` is therefore gauge-for-gauge and tag-for-tag identical to Micrometer 1.16's binder (same `Tags.concat(tags, "httpclient", name)` ordering), verified by scraping `/actuator/prometheus`.

The *request* half of that package **has** been migrated to Apache (creed-simple-metrics `CamelConfig`) — see [[creed-simple-metrics]]. That required `httpclient5` **5.6.4**, pinned by **explicit `dependencyManagement` entries in the root pom, not a property**: this project *imports* `spring-boot-dependencies` rather than inheriting it, and a property cannot override an imported BOM — only a direct `dependencyManagement` entry wins. `httpcore5` follows transitively to 5.4.3; no separate pin needed. Expose via `management.endpoints.web.exposure.include: health,info,metrics`.
